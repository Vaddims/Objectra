...
export class Objectra<
  ContentType extends Objectra.Content<any> = Objectra.Content<any>,
  InstanceType = any,
> {
  ...
  public get childObjectras() {
    ...
    const contentEntries = Object.entries(
      this.content as IndexableObject<Objectra>,
    );
    const objectraEntries: ObjectraCluster.Entry[] = contentEntries.reduce(
      (entries, [key, subObjectra]) => {
        return entries.concat([subObjectra, { path: [key] }]);
      },
  ...
  public get descendantObjectras() {
    ...
    function registerEntry(
      ...
      const contentEntries = Object.entries(
        objectra.content as IndexableObject<Objectra>,
      );
  ...
  public contentIsInstanceOf(constructor: Constructor): boolean {
    ...
    const contentTransformator = Transformator.findAvailable(
      this.identifier,
      this.overload,
    );
  ...
  public compose(): InstanceType {
    ...
    const resolvedObjectraReferenceDefinition = new Map<
      Objectra,
      Objectra.Reference
    >();
    ...
    const composeValueWithinContext: Objectra.Compositor = (objectra) => {
      if (objectra.isReferenceConsumer) {
        ...
        const resolvedReferenceDefinition =
          getResolvedReferenceDefinition(objectra);
        ...
        if (declarationObjectra) {
          ...
          const definitionInstance =
            composeValueWithinContext(declarationObjectra);
        ...
        awaitingDeclarationLocatedConsumer.set(objectra, [
          ...contextPropertyKeyPath,
        ]);
      ...
      const transformator = Transformator.get(
        objectra.identifier!,
        objectra.overload,
      );
      const constructInstanceWithArguments =
        createInstanceConstructor(transformator);
      ...
      if (transformator.instantiate) {
        const preinstance = transformator.argumentPassthrough
          ? constructInstanceWithArguments()
          : undefined;
        const instance = transformator.instantiate({
          ...createInstantiationBridge(transformator),
      ...
      const useForceArgumentPassthrough =
        transformator.ignoreDefaultArgumentBehaviour &&
        transformator.argumentPassthrough;
      if (
        isES6Primitive(objectra.content) &&
        (typeArgumentLenght === 1 || useForceArgumentPassthrough)
      ) {
      ...
      if (!FunctionTypeDeterminant.isConstructor(transformator.type)) {
        throw new TransformatorError.TransformatorInvalidTypeError(
          transformator.type,
        );
      }

      // ! Search to the nearest transformator with a defined instantiation function
      const superTransformators = Transformator.getSuperTransformators(
        transformator.type,
      );
      const instantiationTransformator = Array.from(superTransformators).find(
        (transformator) => transformator.instantiate,
      );
      if (!instantiationTransformator) {
        throw new TransformatorError.TransformatorAncestorsMissingError(
          transformator.type,
        );
      }
      ...
      if (transformator.useSerializationSymbolIterator) {
        ...
        const iteratorContents = instantiationTransformator.instantiate!(
          createInstantiationBridge(transformator),
        ) as any[];
      ...
      if (transformator["argumentPassthroughPropertyKeys"].length > 0) {
        ...
        const content =
          instantiationTransformator.instantiate!(instantiationOptions);

        const argumentPassthroughPropertyKeys =
          transformator["argumentPassthroughPropertyKeys"];
        const constructorArguments = argumentPassthroughPropertyKeys.map(
          (key) => content[key],
        );
        ...
        const redefineKeys = Object.keys(content).filter(
          (key) =>
      ...
      function createInstantiationBridge(
        transformator: Transformator,
      ): Transformator.InstantiationBridge<any, any> {
        return {
          value: objectra,
    ...
    function createInstanceConstructor(
      transformator: Transformator<Constructor | Function>,
    ) {
      return function (...typeArguments: unknown[]) {
        try {
          ...
          throw new TransformatorError.InvalidConstructorArgumetsError(
            transformator.type,
            error,
          );
        }
    ...
    function getResolvedReferenceDefinition(
      target: Objectra,
    ): Objectra.Reference | null {
      const resolvedReferences = Array.from(
        resolvedObjectraReferenceDefinition,
      );
    ...
    function injectNewReferenceDefinitionsInto(instanceToInject: any) {
      for (const [
        objectra,
        appearancePath,
      ] of awaitingDeclarationLocatedConsumer) {
        const resolution = getResolvedReferenceDefinition(objectra);
        if (resolution === null) {
          continue;
        }

        const fullRelativePath = [...appearancePath].slice(
          contextPropertyKeyPath.length,
        );
        ...
        for (let i = 0; i < fullRelativePath.length; i++) {
          ...
          if (transformator.useSerializationSymbolIterator) {
            if (
              i + transformator.symbolIteratorEntryDepth ===
              fullRelativePath.length
            ) {
          ...
          if (keyIsLast) {
            if (
              !(
                drilledObject[key] instanceof
                Objectra.UnresolvedReferencePlaceholder
              )
            ) {
  ...
  public toModel(): Objectra.Model {
    const createModel = (objectra: Objectra) => {
      if (
        objectra.identifier &&
        !Transformator.staticExists(objectra.identifier, objectra.overload)
      ) {
        throw new TransformatorError.TransformatorMissingError(
          objectra.identifier,
        );
      }
  ...
  public createBackloopReferenceCommunication(): Objectra.BackloopReference.Communication<ContentType> {
    ...
    const createReference: Objectra.BackloopReference.Creator = <
      T extends CommonObjectra,
    >(
      objectra: T,
    ) => {
    ...
    const resolveReference: Objectra.BackloopReference.Resolver = <
      T extends Backloop.Reference,
    >(
      representer: T,
    ) => {
  ...
  public static from<T>(
    value: T,
    options: Objectra.Decomposition.Options = {},
  ): Objectra<Objectra.Content<T>, T> {
    const objectStrucutreAnalysis = Objectra.analyzeObjectStructure(value);
    const referenceHoistingParentMap = Objectra.getReferenceHoistingParents(
      objectStrucutreAnalysis,
    );
    ...
    const referenceIdentifiers = new Map<Objectra.Reference, number>(
      Array.from(repeatingReferences).map((reference, index) => [
        reference,
        index,
      ]),
    );

    const serializeValueWithinContext: Objectra.Serializator = <T>(
      providedInstance: T,
    ) => {
      ...
      if (providedInstance === null) {
        return new Objectra<Objectra.Content<T>>({
          content: null as Objectra.Content<any>,
        });
      }
      ...
      const instanceIsReferenceDefinition =
        instanceIsReference &&
        repeatingReferences.has(instance) &&
        !referableReferences.has(instance);
      const instanceShouldReinstantiateOnDefinition = instanceIsClassDeclaration
        ? false
        : instanceIsReferenceDefinition;

      if (
        !instanceShouldReinstantiateOnDefinition &&
        repeatingReferences.has(instance) &&
        referableReferences.has(instance)
      ) {
      ...
      const alternativeTransformator =
        alternativeReferenceTransformation.get(instance);

      const instanceConstrucor = instance.constructor as Constructor;
      const instanceTransformator =
        alternativeTransformator ?? Transformator.get(instanceConstrucor);
      const superTransformators =
        Transformator.getSuperTransformators(instanceConstrucor);
      ...
      if (
        instanceTransformator.serialize &&
        !instanceTransformator.ignoreDefaultArgumentBehaviour &&
        !instanceTransformator.instantiate
      ) {
        throw new TransformatorError.TransformatorInstantiatorMissingError(
          instanceTransformator.identifierToString(),
        );
      }

      const highestSerializationTransformator = Array.from(transformators).find(
        (transformator) => transformator.serialize,
      );
      ...
      const hoistingReferences = Array.from(referenceHoistingParentMap)
        .filter(([, parent]) => parent === instance)
        .map(([reference]) => reference);

      const objectraHoistings = hoistingReferences.map(
        serializeValueWithinContext,
      );

      const id = instanceShouldReinstantiateOnDefinition
        ? referenceIdentifiers.get(instance)
        : void 0;
      ...
      const objectra = new Objectra({
        identifier: alternativeTransformator
          ? alternativeTransformator.type
          : instance.constructor,
        content: objectraContent,
        hoistingReferences: objectraHoistings.length
          ? objectraHoistings
          : void 0,
        id,
      });
  ...
  public static fromModel(model: Objectra.Model | string): Objectra {
    const parseModel = (target: Objectra.Model): Objectra => {
      const constructObjectra = (identifier?: Objectra.Identifier) => {
        const hoistings: Objectra[] = Array.from(
          target.h?.map(parseModel) ?? [],
        );
      ...
      if (target.t) {
        const targetStringIdentifier =
          typeof target.n === "string" ? target.n : target.t;

        if (typeof target.n === "string") {
          const transformator = Transformator.getStaticByStringType(
            target.n,
            target.o,
          );
  ...
  public static duplicate<T>(
    value: T,
    options?: Objectra.Duplication.Options,
  ): T {
  ...
  private static isValueReference(
    target: unknown,
  ): target is Objectra.Reference {
    return (
      Objectra.isIterableEntity(target) ||
      typeof target === "function" ||
      typeof target === "symbol"
    );
  }

  public static analyzeObjectStructure(
    value: unknown,
  ): Objectra.ObjectStrucureAnalysis {
    const referenceAppearancePathMap: Objectra.ReferenceAppearancePathMap =
      new Map();
    ...
    function analyzeValue(target: unknown, pathStack: IterableEntity[]) {
      if (
        !Objectra.isValueReference(target) ||
      ...
      if (registeredApearencePaths) {
        ...
        referenceAppearancePathMap.set(target, [
          ...registeredApearencePaths,
          currentPathStack,
        ]);
      } else {
      ...
      if (
        transformator.useSerializationSymbolIterator &&
      ...
      const indexableObjectKeys =
        transformator.getMaskedObjectPropertyNames(target);
  ...
  private static getReferenceCommonParent(
    referenceAppearancePaths: IterableEntity[][],
  ): IterableEntity {
    ...
    do {
      ...
      const parallelPathElements = referenceAppearancePaths.map(
        (path) => path[nextDepth],
      );
  ...
  private static getReferenceHoistingParents(
    objectStrucureAnalysis: Objectra.ObjectStrucureAnalysis,
  ): Map<Objectra.Reference, Objectra.Reference> {
    const { repeatingReferences, referenceAppearancePathMap } =
      objectStrucureAnalysis;
    const referenceHoistingParents = new Map<
      Objectra.Reference,
      Objectra.Reference
    >();

    for (const repeatingReference of repeatingReferences) {
      const referenceAppearancePaths =
        referenceAppearancePathMap.get(repeatingReference)!;
      const referenceParent = Objectra.getReferenceCommonParent(
        referenceAppearancePaths,
      );
      if (referenceParent) {
...
export namespace Objectra {
  ...
  export type Content<T = ContentStructure<Objectra>> = T extends ES3Primitives
    ? T
  ...
  export type Serializator = <T>(
    instance: T,
  ) => Objectra<Objectra.Content<T>, T>;
  export type Compositor = <K, T>(instance: Objectra<K, T>) => T;

  export namespace BackloopReference {
    export type Creator = <T extends Objectra<Objectra.Content<any>>>(
      objectra: T,
    ) => Backloop.Reference<T>;
    export type Resolver = <T extends Backloop.Reference>(
      representer: T,
    ) => Backloop.ReferenceResolve<T>;
    export type Communication<T> = readonly [
      Backloop.Reference<Objectra<T>>,
      Resolver,
    ];
  }

  export type ReferenceAppearancePathMap = Map<
    Objectra.Reference,
    IterableEntity[][]
  >;
  ...
  export type GetContentType<T extends Objectra> =
    T extends Objectra<infer U> ? U : never;
  export type GetInstanceType<T extends Objectra> =
    T extends Objectra<any, infer U> ? U : never;
}
