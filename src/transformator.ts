...
import {
  getConstructorSuperConstructors,
  FunctionTypeDeterminant,
} from "./utils";
...
export type MetaSymbolKeyTemplate =
  `${MetaKeyType.Symbol}${`@${number}` | ""}:${string}`;
export type MetaKeyTemplate = MetaStringKeyTemplate | MetaSymbolKeyTemplate;

type IdentifierInstance<RegistrationIdentifier> =
  RegistrationIdentifier extends Constructor
...
export class Transformator<
  IdentifierType extends Objectra.Identifier = Objectra.Identifier,
  InstanceType = any,
...
> {
  ...
  private readonly branchedProperties: readonly (keyof Transformator.ConfigOptions<
    InstanceType,
    SerializationType
  >)[] = [];
  ...
  // transformers
  public readonly serializator: NonNullable<
    Transformator.Transformers<InstanceType, SerializationType>["serializator"]
  >;
  public readonly instantiator: NonNullable<
    Transformator.Transformers<InstanceType, SerializationType>["instantiator"]
  >;
  public readonly getter: NonNullable<
    Transformator.Transformers<InstanceType, SerializationType>["getter"]
  >;
  public readonly setter: NonNullable<
    Transformator.Transformers<InstanceType, SerializationType>["setter"]
  >;
  ...
  public static readonly dynamicRegistrations = new WeakMap<
    Constructor | Function,
    Transformator
  >();

  private constructor(
    options: Transformator.InitOptions<
      IdentifierType,
      InstanceType,
      SerializationType
    >,
    ...
    this.ignoreDefaultArgumentBehaviour =
      options.ignoreDefaultArgumentBehaviour ?? false;
    this.argumentPassthroughPropertyKeys = Array.from(
      options.argumentPassthroughPropertyKeys ?? [],
    );

    this.propertyExclusionMask = options.propertyExclusionMask ?? [];

    this.useSerializationSymbolIterator =
      options.useSerializationSymbolIterator ?? false;
    ...
    if (
      !this.ignoreDefaultArgumentBehaviour &&
      !this.instantiator &&
      this.argumentPassthrough &&
      typeof this.type === "function" &&
      this.type.length > 1
    ) {
      throw new TransformatorError.InvalidConstructorArgumentQuantityError(
        this.identifierToString(),
      );
    }
  }
  ...
  public get isConstructorIdentified() {
    return (
      !this.isNameIdentified &&
      FunctionTypeDeterminant.isConstructor(this.type as Function)
    );
  }
  ...
  private serializationProxy<T extends InstanceType>(
    bridge: Transformator.SerializationBridge<T>,
  ): Objectra.Content {
    if (!this.serializator) {
      throw new TransformatorError.TransformatorSerializatorMissingError(
        this.identifierToString(),
      );
    }
    ...
    try {
      return this.serializator({
        ...
        useSerializationSymbolIterator:
          instanceTransformator.useSerializationSymbolIterator,
      });
    } catch (error) {
      if (error instanceof RangeError) {
        throw new TransformatorError.TransformatorSelfSerializationError(
          this.identifierToString(),
          error,
        );
      }
  ...
  private instantiationProxy(
    bridge: Transformator.InstantiationBridge<SerializationType, InstanceType>,
  ): InstanceType {
    if (!this.instantiator) {
      throw new TransformatorError.TransformatorInstantiatorMissingError(
        this.identifierToString(),
      );
    }
    ...
    const {
      ...
      instantiate,
      instance,
      initialTransformator = this,
      keyPath,
    } = bridge;
    const [backloopRepresenter, resolve] =
      value.createBackloopReferenceCommunication();

    const representer = backloopRepresenter;
    const instantiateValue = instantiate;
    const getRepresenterObjectra = resolve;
    const getRepresenterValue = <T>(
      endpoint: Backloop.Reference<Objectra<T>>,
    ): T => (<Objectra<T, any>>getRepresenterObjectra(endpoint))["content"]!;
    const instantiateRepresenter = <K>(
      endpoint: Backloop.Reference<Objectra<K>>,
    ): K => instantiateValue(getRepresenterObjectra(endpoint));
    ...
    try {
      ...
      if (error instanceof RangeError) {
        throw new TransformatorError.TransformatorSelfInstantiationError(
          this.identifierToString(),
          error,
        );
      }
  ...
  public configure<
    SerializedStructure,
    RegistrationIdentifier = IdentifierType,
  >(
    options: Transformator.ConfigOptions<
      IdentifierInstance<RegistrationIdentifier>,
      SerializedStructure
    > = {},
  ) {
    ...
    const transformatorIndex = Transformator.staticRegistrations.findIndex(
      (transformator) =>
        transformator.type === this.type &&
        transformator.overload === this.overload,
    );
  ...
  public getSpecificationOptions(): Transformator.SpecificationOptions {
    return {
      ...
      argumentPassthroughPropertyKeys: Array.from(
        this.argumentPassthroughPropertyKeys,
      ),
  ...
  public getMaskedObjectPropertyNames(instance: InstanceType) {
    ...
    ].filter((key) => !this.propertyExclusionMask.includes(key));
  }

  private static splitConfigOptions<V, S>(
    options: Transformator.ConfigOptions<V, S>,
  ): Transformator.ConfigOptionsSplitted<V, S> {
    const { serializator, instantiator, getter, setter, ...specifications } =
      options;
  ...
  private static getIdentifierConstructor(
    identifer: Objectra.Identifier,
  ): Constructor {
  ...
  private static isDescribable<T extends Objectra.Identifier>(
    identifier: T,
    overload?: number,
  ) {
  ...
  public static exists(identifier: Objectra.Identifier, overload?: number) {
    const staticRegistrationExist = Transformator.staticRegistrations.some(
      Transformator.isDescribable(identifier, overload),
    );
  ...
  public static staticExists(
    identifier: Objectra.Identifier,
    overload?: number,
  ) {
    const staticRegistrationExist = Transformator.staticRegistrations.some(
      Transformator.isDescribable(identifier, overload),
  ...
  public static findStaticByStringType(name: string, overload?: number) {
    return Transformator.staticRegistrations.find(
      (transformator) =>
  ...
  public static findStatic<T extends Objectra.Identifier>(
    identifier: T,
    overload?: number,
  ) {
    return Transformator.staticRegistrations.find(
  ...
  public static getStatic<T extends Objectra.Identifier>(
    identifier: T,
    overload?: number,
  ) {
  ...
  public static findAvailable<T extends Objectra.Identifier>(
    identifier: T,
    overload?: number,
  ) {
  ...
  public static getAvailable<T extends Objectra.Identifier>(
    identifier: T,
    overload?: number,
  ) {
  ...
  public static get<T extends Function | Constructor>(
    identifier: T,
    overload?: number,
  ) {
  ...
  public static *getTransformatorsOfSuperConstructor<T extends Constructor>(
    constructor: T,
  ) {
    ...
    for (const transformator of Transformator.staticRegistrations) {
      if (
        typeof transformator.type !== "function" ||
        !FunctionTypeDeterminant.isConstructor(transformator.type)
      ) {
        continue;
      }

      const superConstructors = getConstructorSuperConstructors(
        transformator.type,
      );
      for (const SuperConstructor of superConstructors) {
  ...
  public static getParentTransformator(constructor: Constructor) {
    const superTransfarmators =
      Transformator.getSuperTransformators(constructor);
    return superTransfarmators.next().value ?? null;
  }

  private static createInherited<T extends Objectra.Identifier, V, S>(
    options: Transformator.InheritedInitOptions<T, V, S>,
  ): Transformator<T, V, S> {
    const typeConstructor = Transformator.getIdentifierConstructor(
      options.type,
    );
    const parentTransformator =
      Transformator.getParentTransformator(typeConstructor);
    ...
    const transformator = new Transformator({
      ...
      branchedProperties: Object.keys(
        specifications,
      ) as (keyof Transformator.ConfigOptions<V, S>)[],
  ...
  public static register<RegistrationIdentifier extends Objectra.Identifier>(
    ...
    if (Transformator.exists(identifier, overload)) {
      ...
      throw new TransformatorError.TransformatorRegistrationDuplicationError(
        identifier,
      );
    }
    ...
    return transformator as Transformator<
      RegistrationIdentifier extends string ? any : RegistrationIdentifier,
      unknown,
      unknown
    >;
  }

  public static registerDynamic<T extends Constructor | Function>(
    identifier: T,
  ) {
    const transformator = Transformator.createInherited({
  ...
  public static findSymbol(symbolDescription: string, symbolOverload?: number) {
    const symbolRegistration = Transformator.staticSymbols.find(
      ([symbol, overload]) =>
        symbol.description === symbolDescription && symbolOverload === overload,
    );
  ...
  public static getSymbol(symbolDescription: string, symbolOverload?: number) {
    ...
    if (!symbol) {
      throw new TransformatorError.SymbolRegistrationMissingError(
        symbolDescription,
      );
    }
  ...
  private static createMetaSymbolKey(
    key: string,
    overload?: number,
  ): MetaSymbolKeyTemplate {
    const overloadArgument =
      typeof overload === "undefined" ? "" : (`@${overload}` as const);
    return `${MetaKeyType.Symbol}${overloadArgument}:${key}`;
  }
  ...
  public static decodeMetaKey(metaKey: MetaKeyTemplate) {
    ...
    switch (type) {
      ...
      case MetaKeyType.Symbol:
        const symbolOverload =
          typeof args[0] === "undefined" ? undefined : Number(args[0]);
        return {
          type,
          symbol: Transformator.getSymbol(key, symbolOverload),
        };
    }
  }

  public static getMetaKeyRepresenter(metaKey: string) {
    const decodedMetaKey = Transformator.decodeMetaKey(
      metaKey as MetaKeyTemplate,
    );
  ...
  public static projectMetaKey(target: PropertyKey) {
    ...
    return Transformator.createMetaSymbolKey(
      symbolRegistration[0].description!,
      symbolRegistration[1],
    );
  }

  // ! #region Decorators

  private static readonly awaitingRegistrationFieldResolvers: Transformator.RegistrationCallbackQueueSet =
    new Set();
  public static Register<
    T = unknown,
    S = any,
    K extends Constructor = T extends Constructor ? T : Constructor<T>,
  >(options: Transformator.ConfigOptions<IdentifierInstance<K>, S> = {}) {
    return (constructor: K, context: ClassDecoratorContext): void => {
      ...
      const parentOptions =
        Transformator.getParentTransformator(
          constructor,
  ...
  private static conditionalTransformationException(
    targetMapping?: Transformator.PropertyTransformationMapping,
  ) {
    return function <C, V>(
      _: undefined,
      ...
      const resolver: Transformator.RegistrationCallback = (
        transformatorOptions,
        type,
      ) => {
        const newTransformatorOptions: Writeable<
          Transformator.ConfigOptions<any, any>
        > = {};

        const parentPropertyExclusionMask =
          transformatorOptions.propertyExclusionMask ?? [];
        const newPropertyExclusionMask = [...parentPropertyExclusionMask];

        const propertyKeyMaskIndex =
          newPropertyExclusionMask.indexOf(propertyKey);
        ...
        if (
          newPropertyExclusionMask.length !== parentPropertyExclusionMask.length
        ) {
          newTransformatorOptions.propertyExclusionMask = [
            ...newPropertyExclusionMask,
          ];
        }
  ...
  public static ConstructorArgument(argumentIndex?: number) {
    return function <T, V>(
      ...
      const resolver: Transformator.RegistrationCallback = (
        options: Transformator.ConfigOptions<any, any>,
        type,
      ) => {
        ...
        if (customOptions.argumentPassthroughPropertyKeys[argumentIndex]) {
          throw new TransformatorError.ConstructorArgumentIndexDuplicationError(
            type,
            argumentIndex,
          );
        }

        customOptions.argumentPassthroughPropertyKeys[argumentIndex] =
          propertyKey;
        return customOptions;
      };
...
export namespace Transformator {
  ...
  export namespace Transformer {
    ...
    export interface InstantiationBridge<SerializedType, Instance> {
      readonly instance?: Instance;
      readonly representer: Backloop.Reference<
        Objectra<Objectra.Content<SerializedType>>
      >;
      readonly instantiateValue: Objectra.Compositor;
      readonly getRepresenterObjectra: Backloop.ResolveRepresenter;
      readonly getRepresenterValue: <T>(
        endpoint: Backloop.Reference<Objectra<T>>,
      ) => T;
      readonly instantiateRepresenter: <T>(
        endpoint: Backloop.Reference<Objectra<T>>,
      ) => T;
      readonly initialTransformator: Transformator;
      readonly keyPath: PropertyKey[];
    ...
    export type Serializator<Instance> = (
      bridge: SerializationBridge<Instance>,
    ) => Objectra.Content;
    export type Instantiator<Serialization, Instance> = (
      bridge: InstantiationBridge<Serialization, Instance>,
    ) => Instance;
  }
  ...
  export interface ConfigOptions<V, S>
    extends SpecificationOptions,
      Transformers<V, S> {}

  export interface RegistrationOptions<T extends Objectra.Identifier, V, S>
    extends ConfigOptions<V, S> {
    readonly type: T;
    readonly overload?: number;
  }

  export type InitOptions<
    T extends Objectra.Identifier,
    V,
    S,
  > = Transformator.RegistrationOptions<T, V, S> & {
  ...
  export interface RegistrationCallback<V = any, S = any> {
    (
      options: ConfigOptions<V, S>,
      type: Objectra.Identifier,
    ): Writeable<ConfigOptions<V, S>>;
  }
}
